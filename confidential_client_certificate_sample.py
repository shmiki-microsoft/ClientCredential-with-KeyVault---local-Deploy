...
# Optional logging for Azure SDK
# #ロガーを取得
...
# Optional logging for MSAL
#logging.basicConfig(level=logging.DEBUG)

config = json.load(open("parameters_raw.json"))
#key vault からシークレットを取得
KVUri = f"https://{config["key_vault_name"]}.vault.azure.net"
credential = DefaultAzureCredential(logging_enable=True)
client = SecretClient(vault_url=KVUri, credential=credential,logging_enable=True,logging_body=True)
private_key = client.get_secret(config["key_vault_secret_name"]).value

# Create a preferably long-lived app instance which maintains a token cache.
app = msal.ConfidentialClientApplication(
    config["client_id"], authority=config["authority"],
    client_credential={"thumbprint": config["thumbprint"], "private_key": private_key},
